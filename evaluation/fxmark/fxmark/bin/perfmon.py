...
class PerfMon(object):
    ...
    def _perf_stat_start(self):
        ...
        self._exec_cmd("sudo /usr/src/linux-nova-4.18/tools/perf/perf stat -a -g -o %s sleep %s &" %
    ...
    def _perf_record_start(self):
        ...
        self._exec_cmd("sudo /usr/src/linux-nova-4.18/tools/perf/perf record -F %s -a -g -o %s &" %
                       (PerfMon.PERF_SAMPLE_RATE, perf_out))

    # perf probe sleepable locks
    def _perf_probe_cleanup(self):
        self._exec_cmd("sudo /usr/src/linux-nova-4.18/tools/perf/perf probe --del \'*\'")

    def _perf_probe_add_trace_points(self, arg0):
        self._perf_probe_cleanup()
        for prob in PerfMon.PROBE_SLEEP_LOCK:
            self._exec_cmd("sudo /usr/src/linux-nova-4.18/tools/perf/perf probe --add \'%s %s\'" % (prob, arg0))
        ...
        if len(arg0) > 0:
            ...
            perf_out = os.path.normpath(
                os.path.join(self.DIR, "%s.perf.sleeplock.data" %
                             self.FILE))
        return ("sudo /usr/src/linux-nova-4.18/tools/perf/perf record %s -F %s -a -g -o %s &" %
    ...
    def _perf_lock_record_start(self):
        ...
        self._exec_cmd("sudo /usr/src/linux-nova-4.18/tools/perf/perf lock record -a -g -o %s &" % perf_out)

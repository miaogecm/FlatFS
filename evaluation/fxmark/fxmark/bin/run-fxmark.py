...
class Runner(object):
    ...
    def __init__(self, \
        ...
        self.HOWTO_MOUNT = {
            ...
            "ext2":self.mount_fs_dax,
            #"ext3":self.mount_anyfs,
            "ext4":self.mount_fs_dax,
            #"ext4_no_jnl":self.mount_ext4_no_jnl,
            "xfs":self.mount_fs_dax,
            "nova":self.mount_fs_nova,
            "pmfs":self.mount_fs_pmfs,
            "mofs":self.mount_fs_mofs,
    ...
    def init_nvm_disk(self):
        #return (os.path.exists(Runner.NVMDEV), Runner.NVMDEV)
        return (True, Runner.NVMDEV)
    ...
    def mount_fs_dax(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
        ...
        if p.returncode is not 0:
            return False
        p = self.exec_cmd(' '.join(["sudo mount -o dax -t", fs,
                                    dev_path, mnt_path]),
                          self.dev_null)
        if p.returncode is not 0:
            return False
    ...
    def mount_fs_nova(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
 
        p = self.exec_cmd(' '.join(["sudo mount -t NOVA -o init", 
                                        dev_path, mnt_path]),
                              self.dev_null)
        if p.returncode is not 0:
            return False
    ...
    def mount_fs_pmfs(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
 
        p = self.exec_cmd(' '.join(["sudo mount -t pmfs -o init", 
                                        dev_path, mnt_path]),
                              self.dev_null)
        if p.returncode is not 0:
            return False
    ...
    def mount_fs_mofs(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
 
        p = self.exec_cmd(' '.join(["sudo mount -t mofs -o init", 
                                        dev_path, mnt_path]),
                              self.dev_null)
        if p.returncode is not 0:
            return False
...
def confirm_media_path():
    ...
    yn = input("All data in %s will be deleted. Is it ok? [Y,N]: "
            % (Runner.NVMDEV))
    if yn != "Y":
...
if __name__ == "__main__":
    ...
    run_config = [
        (Runner.CORE_COARSE_GRAIN,
         ...
         #PerfMon.LEVEL_PERF_RECORD,
         #PerfMon.LEVEL_PERF_LOCK,("nvm", "xfs", "MWRM", "48", "directio"))
	    ("nvm", "mofs", "MRDL", "4", "directio"))

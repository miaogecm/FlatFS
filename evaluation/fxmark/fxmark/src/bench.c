...
static void worker_main(void *arg)
{
	...
	fxmark_debug("worker[%ld] run on cpu[%d]", getpid(), worker->id);

        /* pre-work */
        if (bench->ops.pre_work) {
                err = bench->ops.pre_work(worker);
                if (err) {
			fxmark_debug("worker[%ld] pre_work error on cpu[%d]", getpid(), worker->id);
			goto err_out;
		}
        }

	fxmark_debug("worker[%ld] pre_work done on cpu[%d]", getpid(), worker->id);
	...
	fxmark_debug("worker[%ld] main_work start [%d] on cpu[%d]", getpid(), worker->ready, worker->id);
	...
	fxmark_debug("worker[%ld] main_work done on cpu[%d]", getpid(), worker->id);
